"""
Modified ResNet50 for traffic sign classification.

Architecture (Phase 2.3):
    Custom stem with 3×3 stride-1 conv (instead of 7×7 stride-2) to preserve
    spatial information on 64×64 inputs.  Uses Bottleneck blocks [3,4,6,3]
    matching the standard ResNet-50 depth.
"""
...
class ResNet10(nn.Module):
    """
    ResNet-10 with modified stem for 64×64 traffic sign images.

    Layer config: [1, 1, 1, 1] BasicBlock layers → 10 weighted layers.
    Feature dimension: 512 (same as BasicBlock.expansion=1 × 512 planes).
    """
...
class ResNet50(nn.Module):
    """
    ResNet-50 with a modified stem for small (64×64) traffic sign images.

    Layer config: [3, 4, 6, 3] Bottleneck blocks → 50 weighted layers.
    Feature dimension: 2048 (512 × Bottleneck.expansion).
    """
    ...
    def extract_features(self, x: torch.Tensor) -> torch.Tensor:
        """Return 2048-d pooled features (before fc)."""
        x = self.stem(x)
        x = self.layer1(x)
        x = self.layer2(x)
        x = self.layer3(x)
