...
class SingleMLP(nn.Module):
    ...
    @staticmethod
    def _load_filter_data(path, name="Dataset", target_digits=None):
        """
...
def run_experiment(config, train_ds, val_ds, test_ds):
    ...
    val_roc = roc_curve(val_targets, val_preds)
    ...
    plt.grid(True, alpha=0.3)
    plt.savefig('mnist_roc_curve.png', dpi=150, bbox_inches='tight')
    plt.show()
    print("✓ Saved ROC curve to mnist_roc_curve.png")

    return avg_final_val_loss, val_auc

# Grid Search Configuration - Assignment Requirements
...
for i, conf in enumerate(experiments):
    ...
    print(f"Exp {i+1}: Hidden={conf['hidden_dim']}, LR={conf['lr']}, Batch={conf['batch_size']} -> Val Loss: {loss:.4f}, Val AUC: {auc:.4f}")

# Display Leaderboard
df_results = pd.DataFrame(results)
print("shape:{}".format(df_results.shape))
print("\n" + "="*40)
print(" FINAL LEADERBOARD (Sorted by Best Val AUC)")
print("="*40)
